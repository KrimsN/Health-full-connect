...
| Кто | Ключ | Права |
| --- | --- | --- |
| Телефон | Supabase publishable key | `select`, `insert`, `update` на `health_records`/`sync_runs` |
| MCP-сервер | Postgres-роль `health_reader` | только `select`, включая типизированные view |

`anon` (телефон) имеет `select`, хотя изначально задумывалось «только пишет»: PostgREST
переводит `Prefer: resolution=merge-duplicates` в `INSERT ... ON CONFLICT DO UPDATE`, а
эта команда в самом Postgres требует привилегию `SELECT` и сверяет обновляемые строки с
RLS-политикой `SELECT` (поведение ядра, закрывающее CVE-2017-15099) — write-only роль,
умеющая делать upsert, невозможна в принципе без промежуточной `SECURITY DEFINER`
RPC-функции. Утёкший из APK ключ поэтому может прочитать данные о здоровье — осознанный
компромисс ради простоты для личного сайдлоад-приложения, не автоматически данная
гарантия. `run_sql` в MCP не парсит запрос на предмет опасных операций: безопасность
целиком на стороне грантов роли `health_reader` (только `select`, ни на что другое прав
нет).
